...
class findFaceGetPulse(object):
    ...
    def run(self, cam):
        ...
        if self.find_faces:
            cv2.putText(
                self.frame_out, "按下'C'切換鏡頭" % str(
                    cam),
                (10, 25), cv2.FONT_HERSHEY_PLAIN, 1.25, col)
            cv2.putText(
                self.frame_out, "按下'S'鎖定受測者臉部並進行量測",
                       (10, 50), cv2.FONT_HERSHEY_PLAIN, 1.25, col)
            cv2.putText(self.frame_out, "按下'Esc'關閉程式",
            ...
            x, y, w, h = self.face_rect
            cv2.putText(self.frame_out, "受測者追蹤",
                       (x, y), cv2.FONT_HERSHEY_PLAIN, 1.5, col)
            self.draw_rect(forehead1)
            x, y, w, h = forehead1
            cv2.putText(self.frame_out, "額頭",
                       (x, y), cv2.FONT_HERSHEY_PLAIN, 1.5, col)
            return
        ...
        cv2.putText(
            ...
            (10, 25), cv2.FONT_HERSHEY_PLAIN, 1.25, col)
        cv2.putText(
            self.frame_out, "按下'S'重新開始",
                   (10, 50), cv2.FONT_HERSHEY_PLAIN, 1.5, col)
        cv2.putText(self.frame_out, "按'D'切換數據圖",
